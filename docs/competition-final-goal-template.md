...
```text
在 competition-final 模式下，读取平台提供的 README.md、design-docs/、code/ 和 test-cases/，以 design-docs 与 README 中冻结 REST API 契约为唯一最终真相，自动发现并修复 code/** 中所有可确认的设计实现不一致（P0/P1/P2 全部纳入修复范围，P0/P1 为硬阻塞，P2 也不得默认放弃）；公开 test-cases 只能作为 smoke/canary，不得作为需求来源或隐藏测试反馈替代；最终交付可构建、API 契约兼容、规格覆盖充分、静态/语义 checker 通过、生成规格测试通过、公开 smoke 通过、无未修复可确认不一致且未修改禁止文件的 ShopHub 修复工程。
```
...
Scope:
- Find and repair all confirmed design-implementation inconsistencies across P0, P1 and P2.
- P0/P1 issues are hard blockers.
- P2/low-severity issues are still in scope for full-score quality and must be fixed when they are design-backed and safely repairable.
- Do not treat P2 as optional cleanup by default. A confirmed P2 issue may remain only if it is blocked by design ambiguity, environment failure, or safety constraints, and then it must be explicitly reported as remaining risk.
...
4. Build a requirement-to-code trace matrix linking every API and business requirement across P0/P1/P2 to implementation symbols when feasible.
...
7. Build repair tasks only from spec-backed gaps: every confirmed repair task must include severity, spec_id or API contract id, design_source, expected_behavior, actual_behavior evidence, suspected code files, verification commands, and negative constraints.
8. Repair code/** with minimal patches until all confirmed P0/P1/P2 gaps are closed, or until a blocker is explicitly proven and reported. Prefer service/domain fixes over controller/API changes. Preserve frozen API compatibility.
...
- A repair task is valid only if it is backed by a design/API requirement, checker evidence, generated spec test failure, or traceable code/spec mismatch.
- A public test failure alone must not create a repair task.
- A public smoke failure may create or update a task only when it can be mapped back to a spec_id or frozen API contract item.
- Placeholder locations such as code/#deterministic-check are not acceptable for confirmed P0/P1/P2 tasks.
- P0/P1 tasks must block DONE until fixed. P2 tasks should also be fixed for full-score quality; if any confirmed P2 remains, it must be justified in the report and final_goal_gate should mark it as residual risk rather than silently passing.
...
- Spec IR: spec_ir.json exists, is schema-valid, and contains source-line-backed requirements across all severities.
- Trace coverage: P0 requirements have 100% trace coverage; P1 requirements are fully covered; P2 requirements are covered or explicitly classified as non-executable documentation/style risks.
- Static/semantic consistency: no unresolved confirmed P0/P1/P2 issues remain in API, validation, response schema, error handling, state machine, money formula, pagination/sorting, clock, transaction, repository-filter, failure-isolation, or documentation-visible behavior checkers.
...
- Report: 修复报告.md, .agent-work/goal_status.json and .agent-work/final_goal_report.json exist and contain evidence, including all found/fixed/remaining issue counts by severity.
...
- The final answer must include status DONE, BLOCKED, or STOPPED_BY_SAFETY; issue counts by severity; API contract status; verification commands and results; report path; remaining risk.

If full completion is impossible:
- Do not fabricate DONE.
- Return BLOCKED with exact blocking gates, affected spec_id values, severity, code locations, logs, and safest next repair action.
```
...
```json
{
  ...
  "objective": "repair all confirmed P0/P1/P2 design-implementation inconsistencies in code/** so implementation matches design-docs/** and README frozen REST API contract before final hidden judging",
  "severity_policy": {
    "P0": "hard_blocker_must_fix",
    "P1": "hard_blocker_must_fix",
    "P2": "in_scope_must_fix_when_design_backed_and_safely_repairable",
    "remaining_allowed_only_if": [
      "design_ambiguity",
      "environment_blocker",
      "safety_stop"
    ]
  ...
  "public_tests_policy": {
    ...
    "may_create_tasks_without_spec_mapping": false,
  ...
  "done_gates": [
    ...
    "api_compatibility",
    "spec_ir_valid",
    "trace_coverage_all_severities",
    "static_semantic_consistency_all_confirmed_issues",
...
```text
本作品必须以 competition-final / spec-first 模式运行。目标是读取 README.md 与 design-docs/**，构造可执行规格、冻结 REST API 契约、业务规则、需求到代码 trace matrix、静态/语义 checker 与生成规格测试，并据此修复 code/** 中所有可确认的设计实现不一致（P0/P1/P2 全部纳入修复范围，P0/P1 为硬阻塞，P2 也必须在设计依据明确且可安全修改时修复）。公开 test-cases/** 只能作为最终 smoke/canary，不得作为需求来源、repair task 来源或隐藏测试反馈替代。最终 DONE 必须由 final_goal_gate.py 判定，且至少满足：禁止输入未修改、冻结 API 兼容、Spec IR 有效、全严重级别 trace coverage 达标、静态/语义 checker 无未修复可确认问题、generated spec tests 通过、code Maven test/install 通过、public smoke 通过、forbidden/hardcoding/stability guard 通过、修复报告和机器报告齐全。
```
...
```text
执行时不要从 public black-box failure 开始修复。先从 README.md 和 design-docs/** 构建 Spec IR 与 frozen API contract，再扫描 code/**，建立 requirement-to-code trace matrix，运行 deterministic checkers 和 generated spec tests。repair queue 只能由 spec-backed gaps 生成，并覆盖所有 confirmed P0/P1/P2 gaps；P0/P1 先修且硬阻塞，P2 也要在安全可修时继续修复，不能默认跳过。public test-cases/** 仅在末端作为 smoke 运行；如果失败，必须先映射回 spec_id 或 frozen API contract，不能直接按公开 fixture 修复。每轮 patch 后必须运行对应 spec checker/generated spec test、fresh review、forbidden/hardcoding guard，并更新 evidence artifacts。DONE 只能由 final_goal_gate.py 判定。
```
...
```text
你只处理 spec-backed repair task。若 task 没有 spec_id/API contract id、design_source、expected_behavior、actual_behavior evidence 和可定位 code path，应拒绝修复并要求 orchestrator 回到 spec/trace 阶段。修复范围覆盖 P0/P1/P2：P0/P1 优先且硬阻塞，P2 在设计依据明确且可安全修改时也必须修复。修复只允许修改 code/**，必须保持冻结 REST API 契约，不得硬编码 public test fixture。修复后必须运行 task 指定的 checker 或 generated spec test，并记录修改文件、验证命令、结果和剩余风险。
```
...
```text
final_goal_gate.py 是唯一 DONE 判定器。它必须读取 spec_ir、trace_coverage、consistency_report、checker_reports、generated_spec_test_report、Maven logs、public_smoke_report、forbidden_change_report、hardcoding_guard_report、stability_report 和 修复报告.md。任何未修复的 confirmed P0/P1/P2 spec gap、API drift、禁止文件修改、generated spec test failure、public fixture hardcoding、构建失败、稳定性失败都必须阻止 DONE，除非该问题被明确标记为 design_ambiguity、environment_blocker 或 safety_stop 并进入 BLOCKED/remaining risk，而不是伪造 DONE。
```
...
5. Goal 必须覆盖所有可确认不一致点：P0/P1/P2 全部修复；P0/P1 只是优先级和硬阻塞，不代表 P2 可忽略。
6. Goal 必须有机器可判定 DONE gate，而不是 agent 自我判断。
7. Goal 必须列出禁止修改和禁止行为。
8. Goal 必须要求最终输出 `修复报告.md`、`goal_status.json`、`final_goal_report.json`。
9. Goal 必须允许 BLOCKED，但不允许伪造 DONE。
...
```text
- 满分目标是设计实现一致性，不是只消除高优先级 blocker。
- P2 可能不会导致构建失败，但仍可能影响隐藏评分中的边界、文案、排序、默认值、兼容性或报告质量。
- 正确写法是：P0/P1 优先且硬阻塞，P2 也纳入修复；只有设计依据不足、环境阻塞或安全约束时才记录剩余风险。
```
