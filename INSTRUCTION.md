...
按 plan最多并发两个语义Task。Design-to-code先用fresh `obligation-extractor`只读设计并生成原子义务semantic文件，再由helper绑定源码、ID、session和digest；随后另一个fresh `risk-explorer`逐义务比较代码。Code-to-design直接使用fresh `risk-explorer`。每个scout结束后必须把所有准备关闭的比较单元切成最多4项的blind batches；每个batch由不同fresh `negative-coverage-reviewer`独立复核。任何角色都不写机械envelope或运行helper。
...
- `no_mismatch`只是scout初步主张。Helper生成的blind packet移除原disposition、candidate绑定、搜索摘要和countercheck，只保留source-bound item；再确定性切成每批最多4项。每个batch使用不同fresh provider session并按`work/skills/negative-coverage-reviewer.md`以“先搜索、后窄窗口、逐项完成”的渐进上下文重读当前design/code；所有源码从第一次读取起就必须带offset和不超过240行的limit，不得先整读判断文件大小。Reviewer选择verdict前必须以结构化`execution_accounting`逐项写出入口、推进/转换、所有guard/bound、终止/出口、剩余适用工作和一次替代/补偿路径反查；helper只机械校验六字段非空，不判断其语义。Reviewer只能采用设计原文明确支持的适用域和例外，不得用实现动机、常见实践或推测意图替设计缩小义务；代码限制若留下仍在设计适用域内的反例，就必须质疑。形成candidate所需的直接证据和一次定向替代/补偿路径反查充分后立即结束当前item，不得继续搜索实现理由。质疑或证据不足生成普通`uncertain`/差异candidate，只有blind reviewer独立upheld的项才可最终关闭。Scout notes只保留在原coverage供后续审计，不进入blind上下文。每slice最多12条仅约束Scout初始输出，不得丢弃blind reviewer新增的真实分歧；后续全局selector仍最多选择12项深入调查。Helper不以regex、关键词、数值或任何领域模式判断语义。
...
Risk Explorer返回semantic candidates和semantic coverage后，由orchestrator先执行：

```bash
python3 ${WORK_ROOT}/tools/scripts/negative_review.py prepare \
  --state-root ${STATE_ROOT} --sweep-id ${SWEEP_ID} \
  ...
  --output ${STATE_ROOT}/semantic/negative-reviews/${SWEEP_ID}.packet.json
```

`prepare`返回`review_batch_count`。对每个`BATCH_ID`先提取只包含该batch的输入：

```bash
python3 ${WORK_ROOT}/tools/scripts/negative_review.py batch \
  --packet ${STATE_ROOT}/semantic/negative-reviews/${SWEEP_ID}.packet.json \
  --batch-id ${BATCH_ID} \
  --output ${STATE_ROOT}/semantic/negative-reviews/${SWEEP_ID}.${BATCH_ID}.packet.json
```

为每个batch启动不同fresh model，遵循`${WORK_ROOT}/skills/negative-coverage-reviewer.md`，只读该batch packet、当前design inventory、设计原文和只读代码，写`${STATE_ROOT}/semantic/negative-reviews/${SWEEP_ID}.${BATCH_ID}.raw-review.json`。它不得读取总packet、原semantic candidates/coverage或其他结果。即使batch为空也写`{"reviews":[]}`。全部返回后按batch顺序组装，`--batch-review`与`--reviewer-provider-session-id`一一对应且每个参数重复一次：

```bash
python3 ${WORK_ROOT}/tools/scripts/negative_review.py assemble \
  --packet ${STATE_ROOT}/semantic/negative-reviews/${SWEEP_ID}.packet.json \
  --batch-review ${STATE_ROOT}/semantic/negative-reviews/${SWEEP_ID}.${BATCH_ID}.raw-review.json \
  --reviewer-provider-session-id ${NEGATIVE_REVIEW_PROVIDER_SESSION_ID} \
  --output ${STATE_ROOT}/semantic/negative-reviews/${SWEEP_ID}.review.json
```

然后执行：

```bash
python3 ${WORK_ROOT}/tools/scripts/negative_review.py reconcile \
  --state-root ${STATE_ROOT} --sweep-id ${SWEEP_ID} \
  --semantic-candidates ${STATE_ROOT}/semantic/scouts/${SWEEP_ID}.candidates.json \
  --semantic-coverage ${STATE_ROOT}/semantic/scouts/${SWEEP_ID}.coverage.json \
  --packet ${STATE_ROOT}/semantic/negative-reviews/${SWEEP_ID}.packet.json \
  --review ${STATE_ROOT}/semantic/negative-reviews/${SWEEP_ID}.review.json \
  --scout-provider-session-id ${SCOUT_PROVIDER_SESSION_ID} \
  --candidates-output ${STATE_ROOT}/semantic/scouts/${SWEEP_ID}.reconciled.candidates.json \
  --coverage-output ${STATE_ROOT}/semantic/scouts/${SWEEP_ID}.reconciled.coverage.json
```

Helper要求scout与所有review batches的真实provider session互异，且batch sessions彼此互异；它逐项验证review顺序，并把`challenged`候选绑定回原obligation/anchor，但不判断`upheld/challenged`是否语义正确。之后执行：

```bash
python3 ${WORK_ROOT}/tools/scripts/scout_materializer.py \
  --state-root ${STATE_ROOT} --sweep-id ${SWEEP_ID} \
  --semantic-candidates ${STATE_ROOT}/semantic/scouts/${SWEEP_ID}.reconciled.candidates.json \
  --semantic-coverage ${STATE_ROOT}/semantic/scouts/${SWEEP_ID}.reconciled.coverage.json \
...
Helper会核对全部assigned scope、每个义务/anchor以及所有candidate恰好绑定一次，并验证每个最终`no_mismatch`具有fresh `upheld`记录、每个`challenged`已转为candidate且review artifacts/digests仍current；未精确闭合不能complete。非空handoff在check和merge成功后执行：
