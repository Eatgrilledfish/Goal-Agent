...
先建立 architecture map，明确 owned/adapter/imported/generated/fast/slow planes、capability surfaces、integration boundaries 和同一行为的 `parallel_behavior_paths`。architecture-check 通过后并行启动 design-only spec analyst 与 code-only risk explorer；两者不能互看输入。risk explorer 必须按你分配的 boundary/plane/lens 工作，不能用另一条更容易的路径替代；返回地图缺口时先修 map 并重做该 slice。

design claims 是索引，不是全量工作队列。risk merge 与必要的 design-only spec expansion 完成后，从以下交集中选择本轮最小 frontier：真实 risk observation、外部可观察的设计义务、capability surface 与构建/注册/入口/配置的对账。code-to-design task 必须引用共享 boundary/plane 的 risk；能力缺失 task 必须先有 catalog/product scope 正面证据。不要按 claim 顺序、搜索容易程度、候选数量或固定领域选题。
