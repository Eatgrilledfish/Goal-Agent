...
- `prepare-review` 必须生成 machine-readable agent loop/session/handoff/guardrail/tracing contract，opencode 必须按 contract 维护 ledger 和 verdict trace。
...
queue 还包含 `session`、`agent_loop_contract`、`handoffs`、`guardrails`、`approval_flows` 和 `tracing`。这些不是说明性文本，而是 opencode loop 的执行 contract：长任务恢复时先读 `agent_loop_state.json` 和 `agent_run_ledger.jsonl`，按已有 `agent_review_verdicts.jsonl` 跳过已审阅项，并继续追加 ledger 事件。
...
5. 把关键搜索、文件阅读、命令和反向检查写入 verdict 的非空 `tool_trace`。
6. 把增量进展、失败样本、下一步待办和停止原因追加到 `${AGENT_WORK}/agent_run_ledger.jsonl`。
7. 写 queue 中 `verdict_output` 指向的 JSONL 文件。
...
- 误报控制：至少一个反向检查。
- 工具轨迹：非空 `tool_trace`，包含支撑结论的搜索、阅读或命令步骤。
