...
Plan绑定当前architecture digest。真正互斥的是primary code scope：每个slice的`anchor_paths`必须存在、不得为`.`、与其他slice不得相同或父子重叠，且最多包含6个implementation planes。三类required架构ID必须在plan整体中全部出现；一个横跨多个主代码范围的宽ID可以重复引用，但每次都要有本地anchor与关联plane。每个slice逐值使用完整contract lens portfolio；每片最多输出8条最强异常或不对称observation。使用满足条件的最少focused slices，按顺序每批最多并发两个Task：
...
Claim portfolio最多24条：先每个`required|in_scope`设计组一条，再按behavior-family广度/当前claim数分配剩余槽；同组多槽先覆盖不同section。全部materialized claims必须进入同一review scope。最多六轮、每轮最多4个task，每条accepted claim至少一个task并完成finding+critic后才能coverage。每个已完成risk sweep至少由一个`code-to-design risk backtracking`任务引用。
