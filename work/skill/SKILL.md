...
prepare/read-only snapshot
  ├─ light design inventory → design-origin behavior seeds
  └─ architecture map → disjoint code-only risk sweeps
                 ↓ add code-origin seeds
dual-entry incremental evidence-pair frontier
...
Plan绑定当前architecture digest。真正互斥的是primary code scope：每个slice的`anchor_paths`必须存在、不得为`.`、与其他slice不得相同或父子重叠，且最多包含6个implementation planes。三类required架构ID必须在plan整体中全部出现；一个横跨多个主代码范围的宽ID可以重复引用，但每次都要有本地anchor与关联plane。所有slice的lens并集必须完整覆盖contract portfolio，由模型按当前范围相关性分配。使用满足条件的最少focused slices，按顺序每批最多并发两个Task：
...
Inventory 必须覆盖每个 manifest group，但不生成完整义务队列。每个独立`behavior_family`都是design-origin frontier的轻量seed，保留当前输入动态发现的领域术语；superseded文档中的兼容/迁移行为必须映射到替代规范或明确记录歧义，不能静默消失。Catalog link 只是 provenance；`required/in_scope` 与 capability commitment 需要 supplied design 的正面 scope evidence。
...
每个`required|in_scope`设计组至少物化一条原子claim，累计review scope最多24条。最多六轮、每轮最多4个task，把每条accepted claim调查并critic后才能coverage。只要存在适用设计，至少包含一个`design-to-code obligation tracing`或`capability-absence reconciliation`任务；每个已经产生有效observation的已完成risk sweep至少输送一个引用该sweep observation的`code-to-design risk backtracking`任务。选择时先覆盖不同document group、behavior family、execution plane和mode，再深挖同一域。
...
Critic raw handoff只含这些模型字段。相同 finding/evidence 只有一个当前 review；新 evidence才允许 revision。`needs_more_evidence` 必须给具体可执行问题。只有applicability=supported、actual_conflict=yes，且义务为binding/declared capability或有正面采用证据的optional branch时才可confirm；技术上合规但不理想、未采用MAY、或单项行为合规但仅聚合层面可疑必须reject/needs_more_evidence。Self-check/merge另行确定性写入 `input_digests.claim_sha256/finding_sha256/probe_sha256` 和 `evidence_critic_prompt_version=evidence-critic-v3`；模型不得手填，任一输入摘要变化后旧 critic机械失效并需要 fresh复审。
...
`remaining_scoped_claims`非空时coverage必须失败并回到调查阶段，不能把accepted claim降级成gap。`supplement_rounds`只能0/1，仅用于此前未物化的具体gap。只有coverage trace同时`passed=true,closed=true`才能进入Final Judge；`passed=true,closed=false`不得由checkpoint文字覆盖。Scope外仍可保留`remaining_gaps`。
